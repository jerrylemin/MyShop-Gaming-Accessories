...
| Category CRUD and category safety rules | COMPLETE | `Views/Pages/ProductsPage.xaml`, `Views/Pages/ProductsPage.xaml.cs`, `ViewModels/ProductsViewModel.cs`, `Repositories/CategoryRepository.cs`, `Models/CategoryListItem.cs` |
...
| Order search, sorting, filtering, and paging | COMPLETE | `ViewModels/OrdersViewModel.cs`, `Models/OrderQueryOptions.cs`, `Models/OrderSortOption.cs`, `Models/OrderStatusFilterOption.cs`, `Repositories/OrderRepository.cs` |
| Stock synchronization on orders | COMPLETE | `Repositories/OrderRepository.cs` |
| Revenue and profit reports by day, week, month, and year | COMPLETE | `Views/Pages/ReportsPage.xaml`, `ViewModels/ReportsViewModel.cs`, `Services/ReportingService.cs`, `Models/ReportsSnapshot.cs`, `Models/OrderItem.cs` |
...
| Product image asset pipeline | COMPLETE | `scripts/build_gaming_accessory_seed_assets.py`, `Assets/_source_product_images`, `Assets/GamingProducts` |
...
- Reports now aggregate both revenue and realized profit from paid order items using the stored unit cost price snapshot on each order line.
